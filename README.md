# Awesome LLM for Scientific Discovery 
 
> 本项目系统汇集了基于大语言模型（LLM）驱动的科学发现相关的研究论文、工具平台和实践案例，涵盖从问题提出、假设生成、实验设计、科学交流到端到端自动科学发现的全过程。旨在为科研工作者提供一个集中、系统、更新及时的资源库。 
 
--- 
 
## 目录 
 
- [问题提出 (Problem Generation)](#问题提出-problem-generation) 
- [假设生成 (Hypothesis Generation)](#假设生成-hypothesis-generation) 
- [实验设计与执行 (Experiment Design & Execution)](#实验设计与执行-experiment-design--execution) 
- [科学交流与展示 (Scientific Communication & Presentation)](#科学交流与展示-scientific-communication--presentation) 
- [端到端科学发现系统 (End-to-End Discovery Systems)](#端到端科学发现系统-end-to-end-discovery-systems) 
- [评估与验证 (Evaluation & Validation)](#评估与验证-evaluation--validation) 
- [领域应用 (Domain Applications)](#领域应用-domain-applications) 
- [平台与工具链 (Platforms & Toolchains)](#平台与工具链-platforms--toolchains) 
- [挑战与风险 (Challenges & Risks)](#挑战与风险-challenges--risks) 
- [贡献指南 (Contributing)](#贡献指南-contributing) 
- [许可证 (License)](#许可证-license) 
 
--- 
 
## 问题提出 (Problem Generation) 
 
- **SciQAG: A Framework for Auto-Generated Science Question Answering Dataset with Fine-grained Evaluation** (2024-07-10) 
  自动生成科学问题与答案数据集框架，细粒度评估方法。  [Paper](https://arxiv.org/html/2405.09939v1) ![](https://img.shields.io/badge/arXiv-2024.05-red?style=flat-square&logo=arxiv) [Code](https://github.com/MasterAI-EAM/SciQAG)
 
- **LBD + Template Questioning** 
  结合文献基础发现和模板提问自动生成科学问题。 
 
- **SciPIP: An LLM-based Scientific Paper Idea Proposer** (2025-02-17) 
  基于大语言模型的科研论文创意自动生成系统。 
 
- **CoQuest: Exploring Research Question Co-Creation with an LLM-based Agent** (2024-03-20) 
  研究问题协同生成的LLM代理框架。 
 
## 假设生成 (Hypothesis Generation) 
 
- **Improving Scientific Hypothesis Generation with Knowledge Grounded LLMs** (2024-11-04) 
  知识驱动的科学假设自动生成方法。 
 
- **Iterative Hypothesis Generation for Scientific Discovery with Monte Carlo Nash Equilibrium Self-Refining Trees** (2025-03-25) 
  迭代生成科学假设的博弈论树模型。 
 
- **SciAgents: Automating Scientific Discovery through Multi-Agent Intelligent Graph Reasoning** (2024-09-09) 
  基于多智能体图推理的自动科学发现。 
 
## 实验设计与执行 (Experiment Design & Execution) 
 
- **AutoSciLab: A Self-Driving Laboratory for Interpretable Scientific Discovery** (2024-12-16) 
  自动驾驶实验室平台，用于可解释科学实验。 
 
- **AutoOED: Automated Optimal Experiment Design Platform** (2021-04-13) 
  优化实验设计的自动化平台。 
 
- **BO-Muse: A Human Expert and AI Teaming Framework for Accelerated Experimental Design** (2023-03-30) 
  人机协同加速实验设计框架。 
 
## 科学交流与展示 (Scientific Communication & Presentation) 
 
- **CycleResearcher: Improving Automated Research via Automated Review** (2024-10-28) 
  通过自动评审提升科研自动化。 
 
- **ScholarCopilot: Training LLMs for Academic Writing with Accurate Citations** (2025-04-01) 
  训练大语言模型辅助精准学术写作。 
 
- **Paper2Poster: Towards Multimodal Poster Automation from Scientific Papers** (2025-05-27) 
  自动化从论文生成多模态海报。 
 
## 端到端科学发现系统 (End-to-End Discovery Systems) 
 
- **The AI Scientist: Towards Fully Automated Open-Ended Scientific Discovery** (2024-08-12) 
  全自动科学发现系统。 
 
- **Dolphin: Moving Towards Closed-loop Auto-research through Thinking, Practice, and Feedback** (2025-01-07) 
  闭环自动科研系统。 
 
- **Agent Laboratory: Using LLM Agents as Research Assistants** (2025-01-08) 
  利用大语言模型代理辅助科研。 
 
## 评估与验证 (Evaluation & Validation) 
 
- **EarthSE: A Benchmark for Evaluating Earth Scientific Exploration Capability of LLMs** (2025-05-22) 
  地球科学领域LLM能力基准。 
 
- **MLR-Bench: Evaluating AI Agents on Open-Ended Machine Learning Research** (2025-05-26) 
  机器学习研究AI代理评测平台。 
 
## 领域应用 (Domain Applications) 
 
- **生物与生命科学**：蛋白质结构预测、药物发现等。 
- **化学与材料科学**：催化剂设计、新材料发现。 
- **物理与数学**：公式自动发现、数学推理。 
- **气候与地球科学**：气候预测模型、地震监测。 
- **交叉学科与社会科学**：社科事件分析、金融预测。 
- **计算机科学与人工智能**：AI算法优化、模型设计。 
 
## 平台与工具链 (Platforms & Toolchains) 
 
- **AlphaEvolve、ChemCrow、AlphaFold** 等自动科研平台。 
- **SciBench、PaperRobot** 等数据与评估平台。 
 
## 挑战与风险 (Challenges & Risks) 
 
- 数据和知识偏差问题。 
- 推理结果的可验证性。 
- 自动化带来的黑箱风险。 
- 人机协同协议与角色设计。 
- 多学科知识交叉融合难题。 
 
## 贡献指南 (Contributing) 
 
欢迎各位贡献科研论文总结、代码、工具、评测资源！ 
请参考 [CONTRIBUTING.md](./CONTRIBUTING.md) 了解投稿流程与规范。 
 
## 许可证 (License) 
 
本项目采用 [MIT License](./LICENSE) 许可证，欢迎自由使用和修改。 
